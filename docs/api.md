...
| --- | --- | --- | --- |
| `GET /api/events` *[s]* | – | SSE stream of the chat's agent events (`retry: 1000`, then one `data:` frame per event). Queue frames are `{ kind: "queue", action, ids[], queued[], bytes }`; completed assistant messages emit `{ kind: "message-meta", role, timestamp, durationMs }`; `attached` carries `awaitingInput` and `queuedPrompts[]`, while `rekey` carries `queuedPrompts[]`; usage frames carry the canonical `metrics`. Attachments are metadata only, never base64. | – |
| `GET /api/state` *[s]* | – | `{ key, sessionFile, cwd, current, thinkingLevel, thinkingLevels, totals, metrics, streaming, awaitingInput, queuedPrompts, platform, chatArchiving }` (see the `StatePayload` typedef). `awaitingInput` distinguishes an open turn paused on a form from active model work. Queue entries are `{ id, type, text, attachments: [{ mimeType, bytes }], bytes }`, without attachment data. | – |
...
| `GET /api/history` *[s]* | – | `{ key, messages[], turnModel, live[], streaming, awaitingInput }`; every message carries an `entryId` for forking and a `timestamp`. The assistant message that closes a run also carries `durationMs`, measured from its user message to persisted completion. Persisted skill calls expose only name and arguments, and reasoning uses the same provider-neutral block shape as live SSE. | – |
...
| `GET /api/agent-bootstrap` *[s]* | – | `{ cwd, effectivePrompt, files[], toolsMode, tools[], commands[] }`; `effectivePrompt` is the exact generated prompt and file contents are returned only for editable prompt resources up to 512 KiB | – |
...
| `POST /api/agent-bootstrap/file/reset` *[s]* | `{ id }` | `{ ok, restored, bootstrap }`; restores the first state saved before a UI edit, or removes a file that did not originally exist | `400` read-only/symlink resource · `404` id absent from the current catalog |
